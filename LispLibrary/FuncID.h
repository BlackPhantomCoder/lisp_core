...
inline func_data make_func_data();

template<class T>
inline func_data make_range_nbifunc_data();
...
void free_pool_func(Func* func) {
	auto* cfunc = (T*)func;
	*cfunc = T();
	CoreData::get_pool<T>().free(cfunc);
}
template <class T>
void caller_before_args(void* obj, CoreEnvironment& env) {
...
inline func_data make_func_data()
{
	...
	auto data = func_data{ func_data::core_type::undefined };
	...
	if (std::is_base_of_v<RangeNBiFunc, T>) {
		data.core_type = func_data::core_type::range_nbifunc;
	}
	else if (std::is_base_of_v<RangeBiFunc, T>) {
